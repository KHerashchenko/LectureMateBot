...
def upload_file_to_s3(client, file_path, video_id):

    key = f'{video_id}/{file_path.split("/")[-1]}'  # Construct the key with the video_id folder
    client.upload_file(file_path, bucket_name, key)
    print(f'The file {file_path} was uploaded to s3 bucket: {key}')


def download_file_from_s3(client, video_id, file_name):
    destination_path = f'/tmp/{file_name}'
    key = f'{video_id}/{file_name}'  # Construct the key with the video_id folder
